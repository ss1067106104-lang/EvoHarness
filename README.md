...
|[长程任务评测](wiki/评测部分.md)|GAIA、HLE 评测流程和结果说明|
|[在线 Skills 评测实现](在线Skills评测实现说明.md)|在线评测的架构、流程与实现细节|

> 仓库不包含第三方评测数据集和论文 PDF。运行相关评测前，请从数据集官方渠道获取资源并遵守其许可证与使用条款。
...
* 只提交 `.env.example`，不要提交包含真实密钥的 `.env`。
* `.bear/sessions/`、`.bear/skill-evolution/`、长期记忆和工具输出均属于本地运行数据，不应公开。
* 当前项目尚未附带开源许可证。公开发布前，请根据你的授权意图选择并添加 `LICENSE`；未添加许可证时，默认不代表允许他人复制、修改或再分发。
